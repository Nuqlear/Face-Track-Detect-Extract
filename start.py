...
def main():
    ...
    )  # you need to keep a balance between performance and fluency
    margin = (
        args.margin
    )  # if the face is big in your video ,you can set it bigger for tracking easiler
    ...
    with tf.Graph().as_default():
        with tf.compat.v1.Session(
            ...
            for filename in os.listdir(videos_dir):
                ...
                if (
                    suffix != "mp4" and suffix != "avi"
                ):  # you can specify more video formats if you need
                    continue
                ...
                while True:
                    ...
                    if c % detect_interval == 0:
                        ...
                        faces, points = detect_face.detect_face(
                            r_g_b_frame, minsize, pnet, rnet, onet, threshold, factor
                        )
                        logger.info(
...
def parse_args():
    ...
    parser.add_argument(
        "--output_path", type=str, help="Path to save face", default="facepics"
    )
    parser.add_argument(
        "--detect_interval",
    ...
    parser.add_argument("--margin", help="add margin for face", type=int, default=10)
    parser.add_argument(
        "--scale_rate",
    ...
    parser.add_argument("--no_display", help="Display or not", action="store_true")
