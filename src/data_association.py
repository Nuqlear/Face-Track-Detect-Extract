...
def associate_detections_to_trackers(detections, trackers, iou_threshold=0.25):
    ...
    matches = []
    for row_ind, col_ind in zip(*matched_indices):
        if iou_matrix[row_ind, col_ind] < iou_threshold:
            unmatched_detections.append(row_ind)
            unmatched_trackers.append(col_ind)
        else:
            m = m = np.array([[row_ind, col_ind]])
            matches.append(m)
